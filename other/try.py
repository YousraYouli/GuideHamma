...
points_properties = {tuple(feature["geometry"]["coordinates"]): feature["properties"] for feature in points_data["features"]}
...
def route():
    ...
    # Create features for each point with properties
    point_features = []
    for pt in path:
        properties = points_properties.get(pt, {})
        properties.update({"is_node": True})
    ...
    geojson_path = geojson.FeatureCollection([line_feature] + point_features)
